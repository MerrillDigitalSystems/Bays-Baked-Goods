...
/**
 * NOTE: product-derived views (checkout catalog, Product schema, menu summaries,
 * price displays) now live in src/lib/content.ts, which treats the arrays above
 * as the SEED for the runtime store at content/products.json. Once Bailey saves
 * an edit in /admin, the store wins  -  edits here only matter for fresh installs.
 */
